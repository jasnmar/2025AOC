...
function computeDistanceBetweenCoordinates(
...
function removeDuplicates(arr: number[]): number[] {
  const retArr: number[] = [...new Set(arr)]
  retArr.sort((a, b) => a - b)
  return retArr
}
function makeCoordinateMap(coordinateArray: [number, number, number][]) {
  const distanceArray: [number, number, number][] = []
...
function checkForBox(nextBox1: number, circuitMap: [number, number][]): number {
  let foundIndex = -1
...
function mergeCircuits(
  circuitIndex1: number,
  circuitIndex2: number,
  circuitMap: number[][]
) {
  const firstCircuit: number[] | undefined = circuitMap[circuitIndex1]
  const secondCircuit: number[] | undefined = circuitMap[circuitIndex2]
  if (firstCircuit && secondCircuit) {
    ...
    circuitMap[circuitIndex1] = combinedCircuit as [number, number]
...
function connectCircuits(
  ...
  if (arr[0] !== undefined) {
    ...
    const secondElem = arr[0][2] as number
      ...
      if (currentArrElement) {
        ...
        if (existingCircuitIndex1 === -1 && existingCircuitIndex2 === -1) {
          ...
          if (existingValue) {
            ...
            circuitMap[circuitIndex] = removeDuplicates(existingValue) as [
              number,
              number
            ]
          }
