const input = "82853534-82916516,2551046-2603239,805115-902166,3643-7668,4444323719-4444553231,704059-804093,32055-104187,7767164-7799624,25-61,636-1297,419403897-419438690,66-143,152-241,965984-1044801,1-19,376884-573880,9440956-9477161,607805-671086,255-572,3526071225-3526194326,39361322-39455443,63281363-63350881,187662-239652,240754-342269,9371-26138,1720-2729,922545-957329,3477773-3688087,104549-119841";
    ...
    if (nsArray.every((val) => val === firstVal)) {
        ...
        if (restArray.length > 0) {
            ...
            if (filteredNumber.length > 1) {
                ...
                const repeatLocation = restArray.findIndex((digit) => digit === firstVal);
                //if the repeat location is the 2nd digit, repeat digits will find it
                if (repeatLocation === 0)
                    return false;
                const repeatString = numString.slice(0, repeatLocation + 1);
                if (numString.length % repeatString.length != 0)
                    return false;
                let index = 0;
                const repeatArray = [];
                while (index * repeatString.length < numString.length) {
                    repeatArray.push(numString.slice(repeatString.length * index, repeatString.length * index + repeatString.length));
                    index++;
                }
