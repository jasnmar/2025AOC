...
function convertBatteryStringToNumberArray(batteryString) {
...
function findLargestNumberInArray(valueArray) {
    const largest = Math.max(...valueArray);
    // console.log("largest: ", largest)
    return largest;
}
function convertArrayOfNumbersToNumber(numArray) {
    const result = parseInt(numArray.join(''));
    return result;
}
function fillArrayValues(arr) {
    const retArr = [];
    arr.forEach((item) => {
        retArr.push(item);
    });
...
function findLargest12Numbers(numArray) {
    let counter = 11;
    if (numArray.length < counter)
        return [];
    ...
    const resultsArray = [];
    let arrayStart = 0;
    let arrayFront = numArray;
    let trackingArray = numArray;
    while (counter > -1) {
...
function one() {
    ...
    if (fileContents.length > 0) {
        ...
        const batterySelection = batteriesArray.map((battery) => {
            ...
            const largestCount = batteryValueArray.filter((batteryVal) => batteryVal === largest);
            ...
            const largestIndex = batteryValueArray.findIndex((value) => value === largest);
            ...
            const secondLargestAfter = findLargestNumberInArray(afterBatteryValueArray);
            const secondLargestBefore = findLargestNumberInArray(beforeBatteryValueArray);
        ...
        const total = batteryArray.reduce((acc, num) => acc + (num || 0), 0);
        console.log("total: ", total);
    }
}
main();
